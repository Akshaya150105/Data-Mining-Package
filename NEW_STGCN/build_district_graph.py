...
Builds a district graph for STGCN using ONLY DISTANCE:
  1) Distance-based weights from district centroid distance
  2) No similarity graph
  3) W_final = W_distance
...
def save_summary(districts, unmatched, Wd, sigma2, epsilon):
    lines = [
        "=" * 60,
        "DISTRICT GRAPH BUILD SUMMARY",
        "=" * 60,
        ...
        "  graph_output/district_order.csv",
        "  graph_output/district_mapping.csv",
        "  graph_output/W_distance_heatmap.png",
    ]
...
def main(shp_path, db_path, sigma2, epsilon):
    print("=" * 60)
    print("BUILD DISTRICT GRAPH - DISTANCE ONLY")
    print("=" * 60)
    print(f"sigma2={sigma2}  epsilon={epsilon}")
    ...
    print("\n[5] Computing normalized Laplacian...")
    L = normalised_laplacian(W_distance)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Build district graph for STGCN (distance only)")
    parser.add_argument("--shp", required=True, help="Path to district shapefile / geojson")
    parser.add_argument("--db", required=True, help="Path to aadhar.duckdb")
    parser.add_argument("--sigma2", type=float, default=0.1,
                        help="Gaussian spread for distance kernel")
    parser.add_argument("--epsilon", type=float, default=0.1,
    ...
    try:
        main(args.shp, args.db, args.sigma2, args.epsilon)
    except Exception as e:
        print(f"\nERROR: {e}")
        sys.exit(1)