...
#     fig, ax = plt.subplots(figsize=(8, 4.5), facecolor="#f9f8f6")
#     ax.set_facecolor("#ffffff")

#     ax.plot(train_losses, linewidth=2.2, label="Train loss")
#     ax.plot(val_losses, linewidth=2.2, label="Validation loss")

#     ax.set_title("STGCN Training Loss Curve", fontsize=13, fontweight="bold", loc="left", pad=10)
#     ax.set_xlabel("Epoch")
...
#         d_mae, d_rmse = compute_metrics(y_n, p_n)
...
def make_sequences(X, weeks, target_idx, t_in=8, t_out=1):
    inputs, targets, target_weeks = [], [], []
    T = X.shape[0]
...
class STConvBlock(nn.Module):
    def __init__(self, c_in, c_spatial, c_out, Kt, K, L_tilde, dropout=0.2):
        super().__init__()
        self.t1 = TemporalConv(c_in, c_out, Kt)
        self.gc = ChebConv(c_out, c_spatial, K, L_tilde)
        self.t2 = TemporalConv(c_spatial, c_out, Kt)
...
class STGCN(nn.Module):
    def __init__(self, C, T_out, K, Kt, L_tilde, c_s=8, c_out=32, dropout=0.2):
        super().__init__()
        self.t_out_steps = T_out
        self.block1 = STConvBlock(C, c_s, c_out, Kt, K, L_tilde, dropout=dropout)
        self.block2 = STConvBlock(c_out, c_s, c_out, Kt, K, L_tilde, dropout=dropout)
        self.t_out = TemporalConv(c_out, c_out, Kt)
        self.fc = nn.Linear(c_out, 1)
        self.dropout = nn.Dropout(dropout)
...
def train_model(model, tr_x, tr_y, va_x, va_y, epochs=120, lr=1e-3, batch_size=4):
    tr_x = torch.tensor(tr_x, dtype=torch.float32)
    tr_y = torch.tensor(tr_y, dtype=torch.float32)
    va_x = torch.tensor(va_x, dtype=torch.float32, device=DEVICE)
    va_y = torch.tensor(va_y, dtype=torch.float32, device=DEVICE)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for bx, by in train_loader:
            ...
            epoch_train_loss += loss.item() * bx.size(0)
        ...
        if epoch % 5 == 0 or epoch == 1:
            ...
            print(
                f"Epoch {epoch:3d} | train={epoch_train_loss:.5f} | "
                f"val={vloss:.5f} | lr={current_lr:.6f}"
            )
...
def evaluate(model, te_x, te_y_scaled, te_y_raw, test_weeks, districts, target_scaler, output_dir):
    ...
    district_metrics = pd.DataFrame(district_rows).sort_values("mae", ascending=True)
    district_metrics.to_csv(output_dir / "district_metrics.csv", index=False)
...
if __name__ == "__main__":
    ...
    p.add_argument("--t_in", type=int, default=8)
    p.add_argument("--t_out", type=int, default=1)
    p.add_argument("--k", type=int, default=3)
    p.add_argument("--kt", type=int, default=2)
    p.add_argument("--c_spatial", type=int, default=8)
    p.add_argument("--c_out", type=int, default=32)
