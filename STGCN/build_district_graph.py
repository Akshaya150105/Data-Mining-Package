...
TABLES = [
    "biometric_data_preprocessed_stgcn",
    "enrolment_data_preprocessed_stgcn",
]
...
def load_shapefile(path: str):
    """
    Loads a district boundary file.
    Supports: .shp, .geojson, .json, .gpkg
    """
    path_obj = Path(path)
    ...
    if suffix not in [".shp", ".geojson", ".json", ".gpkg"]:
        raise ValueError(
            f"Unsupported boundary file type: {suffix}. "
            f"Use .shp, .geojson, .json, or .gpkg"
        )

    gdf = gpd.read_file(path)

    if gdf.empty:
        raise ValueError("Boundary file loaded, but it contains no rows.")

    if "geometry" not in gdf.columns:
        raise ValueError("Boundary file has no geometry column.")

    dist_col = next(
        (
            c for c in [
                "DISTRICT", "district", "District", "DISTNAME", "dtname",
                "district_name", "name", "NAME", "District_Name"
            ]
            if c in gdf.columns
        ),
        None
    ...
    if dist_col is None:
        raise ValueError(
            f"No district column found in boundary file. Available columns: {list(gdf.columns)}"
        )
...
def main(shp_path, db_path, alpha, sigma2, epsilon):
    ...
    # 1) load shapefile
    print("\n[1] Loading district boundary file...")
...
if __name__ == "__main__":
    ...
    "--shp",
    required=True,
    help="Path to district boundary file (.shp or .geojson)")
